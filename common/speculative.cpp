...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        // Flatten token-wise work into shared chunks while preserving each row's position and
        // sequence. The encoder is fused into the injection decode (#27310): the target features
        // go straight into batch_inject.embd and one llama_decode encodes and injects them, so
        // there is no encoder output round trip through host memory and no second graph.
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
        ...
        , params(params.draft)
    {
        ...
        // ctx_other is also set for drafts that only borrow tensors from the target (qwen4exp
        // shared heads); only the gemma4 assistant actually shares the target's memory
        char arch_dft[64] = {0};
